...
export const transactionKeys = {
  history: (filters: { from: string; to: string; page: number; limit: number; search: string; kasirId?: string }) => 
    ["admin", "transactions", filters] as const,
};

/** Hook untuk mengambil data riwayat transaksi (Audit Log) */
export function useTransactions(dateFrom: Date, dateTo: Date, page = 1, limit = 10, search = "", kasirId?: string) {
  const from = format(dateFrom, "yyyy-MM-dd");
  const to = format(dateTo, "yyyy-MM-dd");

  return useQuery({
    queryKey: transactionKeys.history({ from, to, page, limit, search, kasirId }),
    queryFn: () => getTransactionHistory({ from, to, page, limit, search, kasirId }),
