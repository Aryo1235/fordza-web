...
export async function GET(req: Request) {
  try {
    ...
    const products = await StockOpnameService.getForOpnameExport({ search });
    ...
    if (formatType === "pdf") {
      ...
      doc.text(`Filter Pencarian: ${search || "Semua Produk Aktif"}`, 14, 30);
    ...
    console.error("GET /api/admin/stock/opname/export error:", error.message);
