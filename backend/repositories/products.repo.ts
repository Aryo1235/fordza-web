...
export const ProductRepository = {
  ...
  async getById(id: string) {
    return await prisma.product.findUnique({
      ...
      select: {
        ...
        shortDescription: true,
        price: true,
        stock: true, // ← field ini yang menyebabkan stok selalu 0 di form edit
        productType: true,
        gender: true,
        isPopular: true,
        isBestseller: true,
  ...
  async update(id: string, data: any, operatorId?: string) {
    return await prisma.$transaction(async (tx) => {
      // 0. Ambil data lama untuk cek stok
      ...
      // 5. CATAT LOG STOK (Jika stok berubah)
  ...
  // --- Kasir: produk aktif dengan stok > 0 (untuk layar POS) ---
        ...
        if (delta !== 0) {
          await tx.stockLog.create({
            data: {
              ...
              type: delta > 0 ? "RESTOCK" : "ADJUSTMENT",
              notes:
                delta > 0 ? "Barang masuk (Bulk)" : "Pengurangan manual (Bulk)",
              operatorId: operatorId || null,
            },
          });
        }
